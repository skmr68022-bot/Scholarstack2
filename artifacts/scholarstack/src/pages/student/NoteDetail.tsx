...
export default function NoteDetail() {
  ...
  const handlePurchase = async () => {
    if (!currentUser) { setLocation(`/auth/${note?.category === "competitive" ? "student" : "student"}`); return; }
    if (!note) return;
    setPaying(true);
    await new Promise(r => setTimeout(r, 1400));
    await addPurchased(note.id, note.price, payMethod);
    setPaying(false);
    setPaidDone(true);
    setTimeout(() => { setShowPayModal(false); setPaidDone(false); }, 1800);
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4">
            <div>
              ...
              {note.original && <span className="text-xs text-green-400 ml-2">
      ...
      {/* Payment Modal */}
      {showPayModal && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => !paying && setShowPayModal(false)}>
          <div className="bg-[#13131a] border border-white/10 rounded-3xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            {paidDone ? (
              <div className="text-center py-4">
                <div className="text-5xl mb-3">🎉</div>
                <h3 className="text-xl font-black text-white mb-1">Payment Successful!</h3>
                <p className="text-sm text-gray-400">You can now access this note.</p>
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between mb-5">
                  <h3 className="font-black text-white text-lg">Complete Purchase</h3>
                ...
                <div className="bg-white/5 rounded-2xl p-4 mb-5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {note.original && <span className="text-xs text-gray-500 line-through">{note.original}</span>}
                  </div>
                </div>
                <p className="text-xs text-gray-400 font-semibold mb-3">Payment Method</p>
                <div className="grid grid-cols-2 gap-2 mb-5">
                  {(["upi", "card", "netbanking", "wallet"] as PayMethod[]).map(m => (
                    <button key={m} onClick={() => setPayMethod(m)}
                      className={`py-2.5 rounded-xl text-xs font-semibold border transition ${payMethod === m ? "bg-violet-600/30 border-violet-500/50 text-violet-300" : "bg-white/5 border-white/10 text-gray-400 hover:text-white"}`}>
                ...
                {payMethod === "card" && (
                  <div className="space-y-2 mb-5">
                    <input placeholder="Card number" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-500 outline-none focus:border-violet-500 transition" />
                    <div className="flex gap-2">
                      <input placeholder="MM/YY" className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-500 outline-none focus:border-violet-500 transition" />
                      <input placeholder="CVV" className="w-20 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-500 outline-none focus:border-violet-500 transition" />
                    </div>
                  </div>
                )}
                <button onClick={handlePurchase} disabled={paying}
                  className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white font-bold text-sm hover:opacity-90 transition disabled:opacity-60">
                  {paying
                ...
                <p className="text-[10px] text-gray-500 text-center mt-3">Secured by Razorpay · SSL Encrypted</p>
              </>
            )}
          </div>
        </div>
