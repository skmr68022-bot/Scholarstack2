  ...
  if (done) return (
    <div className="p-6 flex flex-col items-center justify-center min-h-64 text-center">
      ...
      <div className="flex gap-3">
        <button onClick={() => { setDone(false); setTitle(""); }} className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm font-semibold text-gray-300 hover:bg-white/10 transition">
          Upload More
        </button>
    ...
    <div className="p-6 max-w-2xl">
      ...
      <div className="flex gap-3 mb-5">
        {["PDF", "Video", "Bundle"].map(t => (
          <button key={t} onClick={() => setType(t)}
            className={`flex-1 flex flex-col items-center gap-2 py-5 rounded-2xl border transition ${type === t ? "bg-cyan-600/15 border-cyan-500/40 text-cyan-400" : "bg-white/3 border-white/10 text-gray-400 hover:border-white/20 hover:text-white"}`}>
            <span className="text-3xl">{t === "PDF" ? "📄" : t === "Video" ? "🎬" : "📦"}</span>
            <span className="font-bold text-sm">{t}</span>
            <span className="text-[10px] text-center text-current opacity-60">
      ...
      <div className="space-y-4">
        <div>
          ...
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder={`e.g., Complete ${exam} ${type} 2025`}
